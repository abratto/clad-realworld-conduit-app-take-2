@UC-02 @sign-in
Feature: Sign In

  As a Member
  I want to authenticate with email and password
  So that I obtain a JWT for subsequent requests

  Background:
    Given the system is running
    Given a registered user exists

  @sign-in @happy-path
  Scenario: Sign in with valid email and password
    When the user submits POST /api/users/login with email "ada@test.com" and password "correct-horse-battery-staple"
    Then the response status is 200
    And the response body contains "sessionToken"
    And the response body contains "ada@test.com"
    And the response body contains "ada"

  @sign-in @failure-path @no-state-change
  Scenario: Sign in with unknown email
    When the user submits POST /api/users/login with email "unknown@test.com" and password "anything"
    Then the response status is 401
    And the response body contains "invalid"

  @sign-in @failure-path @no-state-change
  Scenario: Sign in with wrong password
    When the user submits POST /api/users/login with email "ada@test.com" and password "wrongpassword"
    Then the response status is 401
    And the response body contains "invalid"

  @sign-in @failure-path @no-state-change
  Scenario: Sign in with blank email
    When the user submits POST /api/users/login with email "" and password "anything"
    Then the response status is 422
    And the response body contains "can't be blank"

  @sign-in @failure-path @no-state-change
  Scenario: Sign in with blank password
    When the user submits POST /api/users/login with email "ada@test.com" and password ""
    Then the response status is 422
    And the response body contains "can't be blank"

  @contract @post-api-users-login
  Scenario: POST /api/users/login matches the external contract
    Given the account is not locked
    When the user submits POST /api/users/login with email "ada@test.com" and password "correct-horse-battery-staple"
    Then the response status is 200
    And the response body contains "username"
    And the response body contains "email"
    And the response body contains "sessionToken"
