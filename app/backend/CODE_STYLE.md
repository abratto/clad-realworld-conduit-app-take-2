...
- **R4 — `Web` is the sole HTTP entry.** Only classes inside
  `com.conduit.app.infrastructure` may carry Micronaut HTTP annotations
  (`@Controller`, `@Get`, `@Post`, …).
- **R5 — every concept is a `ConceptAgent`.** Every `*Concept` class
  inside `com.conduit.app.concepts` must be assignable to
  `com.conduit.app.engine.ConceptAgent`. This guarantees every action
